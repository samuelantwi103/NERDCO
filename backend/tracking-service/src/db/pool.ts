...
pool.on('error', (err: Error) => console.error('[tracking-db] client error', err.message));
...
pool.query = async function retryQuery(...args: any[]): Promise<any> {
  ...
  for (let attempt = 0; attempt <= 4; attempt++) {
    try {
      ...
      console.warn(`[tracking-db] Retrying (${code || msg.slice(0, 40)}), attempt ${attempt + 1}/4 in ${delay}ms`);
