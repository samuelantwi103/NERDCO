...
export default function OrganizationsPage() {
  ...
  useAutoRefresh(async () => {
    try { setOrgs(await listOrganizations(token)); }
    ...
    loadGoogleMaps(MAPS_KEY).then(() => {
      ...
      orgs.forEach(org => {
        if (org.latitude && org.longitude) {
          ...
          pin.innerHTML = \`\${org.type === 'hospital' ? '🏥' : org.type === 'ambulance_service' ? '🚑' : org.type === 'police_station' ? '🚓' : '🚒'}\`;
          ...
          return (
            <div key={type} style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginBottom: '24px' }}>
              ...
              <div className={styles.grid}>
                {items.map(o => <OrgCard key={o.id} org={o} onEdit={() => handleEdit(o)} onDelete={() => setDeleteId(o.id)} />)}
              </div>
            </div>
          );
        })
...
`;
fs.writeFileSync('src/app/(ops)/admin/organizations/page.tsx', updatedCode);
console.log('Saved addEditOrgs.tsx');
